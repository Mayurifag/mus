...
- [ ] **Foundation: Database & Queues**
    ...
    - [ ] Tune SQLite for concurrency by enabling WAL mode and a busy timeout.
    - [ ] Set up a task queue system (e.g., RQ) using DragonflyDB as the broker.
- [ ] **Core Processing Pipeline**
    - [ ] Implement `watchdog` to monitor the music directory for real-time file changes (`created`, `modified`, `deleted`, `moved`).
    - [ ] On startup, perform an initial full scan that populates the database before the watcher takes over.
