...
- Use `make rebuild-backend-image` to refresh baked backend tools like yt-dlp without deleting volumes. Use `make rebuild-frontend-image` to refresh baked frontend tools like npm. Use `make prod-image` to rebuild the production image locally.
