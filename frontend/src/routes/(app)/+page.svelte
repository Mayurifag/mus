<script lang="ts">
	...
	async function handleScan() {
		...
		try {
			...
			scanMessage = error instanceof Error ? error.message : 'An error occurred while scanning.';
		} finally {
			isScanning = false;
		}
	}
...
<div class="container mx-auto p-4 pb-24">
	<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
		<div>
			...
			{#if trackCount > 0}
				<p class="text-muted-foreground">Found {trackCount} tracks in your library.</p>
			{:else}
				<p class="text-muted-foreground">No tracks found. Use the scan function to add tracks.</p>
			{/if}
		</div>
