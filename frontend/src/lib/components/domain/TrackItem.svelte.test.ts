...
describe('TrackItem component', () => {
	...
	it('renders placeholder for missing cover art', () => {
		const trackWithoutCover = { ...mockTrack, has_cover: false, cover_small_url: null };
		render(TrackItem, { track: trackWithoutCover, index: 0, isSelected: false });

		expect(screen.getByText('No Cover')).toBeInTheDocument();
	});

	it('applies selected styles when isSelected is true', () => {
		const { container } = render(TrackItem, { track: mockTrack, index: 0, isSelected: true });
