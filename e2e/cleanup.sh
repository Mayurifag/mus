...
E2E_LINT_IMAGE_NAME="mus:e2e-lint"
...
if docker images -q "$E2E_PLAYWRIGHT_IMAGE_NAME" | grep -q .; then
    echo "Removing image $E2E_PLAYWRIGHT_IMAGE_NAME..."
    docker rmi "$E2E_PLAYWRIGHT_IMAGE_NAME" 2>/dev/null || true
fi

if docker images -q "$E2E_LINT_IMAGE_NAME" | grep -q .; then
    echo "Removing image $E2E_LINT_IMAGE_NAME..."
    docker rmi "$E2E_LINT_IMAGE_NAME" 2>/dev/null || true
fi
