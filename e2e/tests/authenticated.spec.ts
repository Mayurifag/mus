...
async function checkCoverFiles(
  trackId: number,
  ...
  const checkFileExists = (filePath: string): boolean => {
    try {
      execSync(`docker exec ${containerName} test -f "${filePath}"`, {
        stdio: "ignore",
      });
      return true;
    } catch (error) {
...
async function getFileStats(
  filePath: string,
): Promise<{ mtime: number; exists: boolean }> {
  try {
...
test.describe("Authenticated User Flows", () => {
  test("playback controls work correctly", async ({ page }) => {
    ...
    const midnightEchoesTrack = page
      .locator('[data-testid="track-item"]')
      .filter({ hasText: "The Midnight Echoes" });
    const lunaTrack = page
      .locator('[data-testid="track-item"]')
      .filter({ hasText: "Luna Solaris" });
    ...
    await expect(page.locator(".fixed.bottom-0")).toContainText(
      "Luna Solaris - Cosmic Wanderer",
    );
    ...
    const sourceFile = join(
      musicDir,
      "The Midnight Echoes - Digital Dreams.wav",
    );
    ...
    await expect(page.locator(".fixed.bottom-0")).toContainText(
      "The Midnight Echoes - Digital Dreams",
    );
    await expect(page.locator("audio")).toHaveJSProperty("paused", false);
