...
Mus Next Generation is a modern rewrite of the Mus personal music server. It provides a unified experience across desktop (via Tauri) and web, built with a focus on performance and maintainability.
...
*   **Desktop Shell:** Tauri 2.0 wraps the SvelteKit frontend, providing a native desktop application experience.
...
*   **Purpose:** Packages the SvelteKit frontend into a cross-platform desktop application using system webviews.
*   **Configuration:** Managed via `frontend/src-tauri/tauri.conf.json` (or `Tauri.toml`/`.json5`). Specifies window parameters, build settings (pointing to SvelteKit's `build` directory), and the allowlist for native API access.
*   **Communication:** The SvelteKit frontend running inside Tauri communicates directly with the FastAPI backend via HTTP requests to its API endpoints (e.g., `http://localhost:8000/api/v1/...`). Tauri itself doesn't mediate API calls unless specific Tauri commands are implemented for native functionality.
*   **API Access:** Backend API endpoints intended for Tauri access do *not* require the JWT authentication used for web browser access.
...
*   **SvelteKit Frontend (Web/Tauri) <-> FastAPI Backend:** Communication via JSON REST API calls over HTTP (e.g., `GET /api/v1/tracks`, `POST /api/v1/player/state`).
...
*   **User (Tauri App) -> FastAPI Backend:** Direct API calls without JWT authentication.

## 7. Authentication System (Web Access Only)
...
*   **Tauri Access:** Tauri application bypasses this flow and accesses API endpoints directly without the JWT cookie. API endpoints crucial for core functionality (`/tracks`, `/stream`, `/covers`, `/scan`, `/player/state`) are *not* protected by JWT validation.
...
*   **Web vs. Tauri Auth:** Separate authentication strategy acknowledges the different trust levels and user experience goals for web access vs. the desktop application.
